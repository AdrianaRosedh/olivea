...
type Props = {
  ...
  /**
   * Event dispatched when scroll arrives (used to auto-open a popup).
   * Defaults to the existing Farm To Table behavior.
   */
...
export default function MenuScrollButton({
  ...
  children = "Menu",
  label = "View menu",
  openEventName = "olivea:menu:open", // ✅ default unchanged
}: Props) {
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>) => {
      ...
      const hcss = getComputedStyle(document.documentElement).getPropertyValue(
        "--header-h"
      );
      ...
      const OPEN_EVENT = openEventName; // ✅ now configurable
      const start = performance.now();
      let stable = 0;
      let rafId = 0;
