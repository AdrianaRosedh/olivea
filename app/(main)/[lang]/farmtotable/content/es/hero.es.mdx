        ...
        >
          ...
          <Reveal preset="up" delay={0.12}>
            <div className="w-full text-center">
              {/* Móvil: una sola línea, sin líneas laterales */}
              <span
                className="
                  md:hidden
              ...
              {/* Desktop: líneas laterales y una sola línea */}
            ...
            <div className="flex items-center justify-center gap-2.5 md:gap-4 pb-[max(0px,env(safe-area-inset-bottom))]">
              ...
              <a
                ...
                onClick={(e) => {
                  ...
                  // defocus any active control (prevents focus ring flicker during scroll)
                  const ae = document.activeElement;
                  if (ae && typeof ae.blur === 'function') ae.blur();
                
                  // compute header-safe offset
                  ...
                  // when we arrive (or timeout), fix the hash and open the popup
                  const OPEN_EVENT = 'olivea:menu:open';
                  const start = performance.now();
                  const TIMEOUT = 1800;   // ms
                  const THRESH  = 2;      // px
                  let lastY = -1;
                  let stable = 0;
                  ...
                  const tick = () => {
                    ...
                    // consider "stopped" after 2 consecutive frames at target
                    if (arrived && y === lastY) stable += 1;
                    else stable = 0;
                    ...
                    if ((arrived && stable >= 2) || expired) {
                      if (window.location.hash.slice(1) !== 'menu') {
