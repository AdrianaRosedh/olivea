  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-3xl bg-stone-100/80 ring-1 ring-black/5 p-8 md:p-16 text-center space-y-6">
        ...
        <EditableBilingual label={{ es: "Mensaje", en: "Message" }} as="h2" value={(get("message") ?? { es: "", en: "" }) as { es: string; en: string }} onChange={(v) => set("message", v)} className="text-xl font-serif text-stone-700" placeholder={t({ es: "Mensaje de no encontrado…", en: "Not found message..." })} />
        <div className="inline-block rounded-full bg-[var(--olivea-olive)]/10 px-6 py-2">
          <EditableBilingual label={{ es: "Botón de acción", en: "CTA Button" }} as="span" value={(get("cta") ?? { es: "", en: "" }) as { es: string; en: string }} onChange={(v) => set("cta", v)} className="text-sm font-semibold text-[var(--olivea-olive)]" placeholder={t({ es: "Texto del botón…", en: "CTA text..." })} />
        </div>
      </section>
    </div>
  );
